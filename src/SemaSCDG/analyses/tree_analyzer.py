...
class TreeAnalyzer(object):
    ...
    @staticmethod
    # DEAD FUNCTION (due to angr update)
    def compute_symbolic_pages(state):
        if state is None:
            return 0
        symbolic_pages = 0
        #for page_addr in state.memory._pages:
         #   page = state.memory._pages[page_addr]
    ...
    def compute_generated_symbols(self, path):  # improve performance
        ...
        try:
            ...
            bye = 0  # because some node to merge may have been pruned by toolchain
    ...
    def compute_leaves_info(self):
        ...
        for path in self.symb_tree.simplify().paths():
            ...
            lp["num_malloc_free"] = 0 # don't work
            '''reduce(
                lambda x, y: x+y, 
                map(
